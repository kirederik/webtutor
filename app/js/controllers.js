...
function DerivativeCtrl($scope) {
	...
	$scope.refresh = function(element) {
		...
		window.setTimeout( function() {
			var math = MathJax.Hub.getAllJax(element);
			$(math).each(function(index, value) {
				value.Reprocess();
			});
		}, 100);
	};
	...
	$scope.verifyFx = function() {
		var answer = eval($scope.ans.fx.replace(/\,/g, "."));
		var value = eval($scope.problem.f.eval.replace(/x/g, $scope.problem.f.x));
		if (answer == value) {
			$scope.flash.fx.error = false;
		} else {
			$scope.flash.fx.message = "That's not correct!";
...
ProblemFunction.prototype = {
	...
	, expandDerivative: function() {
		...
		this.derivative.value = eval(this.derivative.eval.replace(/x/g, this.f.x));
